...
class RemoteHost extends events_1.EventEmitter {
    constructor(timeout) {
        ...
        this.launchAppComponent = this.bindMethod('app.launchComponent', fdb_protocol_1.FDBTypes.LaunchComponentParams, fdb_protocol_1.FDBTypes.AppComponent, { minTimeout: 20000 });
    ...
    bindMethod(method, paramsType, resultType, { timeoutEnabled = true, minTimeout = 0 } = {}) {
        return (params) => this.rpc
            .callMethod(method, params, {
            timeout: timeoutEnabled
                ? Math.max(this.timeout, minTimeout)
                : undefined,
        })
            .then((0, jsonrpc_ts_1.decode)(resultType));
    }
    writeToStream(stream, data, { onProgress = (() => { }), } = {}) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const overheadChars = 256;
            const maxDataBytes = (0, fdb_protocol_1.maxBase64DecodedSize)(this.maxMessageSize - overheadChars);
            if (maxDataBytes < 1) {
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (this.supportsPartialAppInstall()) {
                try {
                    ...
                    const uuid = yield (0, componentBundle_1.getAppUUID)(bundleZip);
                    const existingContents = yield this.getInstalledAppContents(uuid, componentBundle);
                    const partialBundle = yield (0, componentBundle_1.makePartialBundle)(bundleZip, existingContents);
            ...
            return this.finalizeStreamingInstall({ stream }).then((result) => (Object.assign({ installType: 'full' }, result)));
        });
    }
    canTakeScreenshot() {
    ...
    takeScreenshot(format, onWrite) {
        return this.screenshotReceiver.receiveFromStream((stream) => this.beginStreamingScreenshotCapture({
            stream: stream.token,
            imageFormat: format,
        }).then(({ length }) => {
            if (onWrite) {
    ...
    getInstalledAppContents(uuid, componentBundle) {
        return this.appContentsListReceiver
            .receiveFromStream((stream) => this.beginStreamingAppComponentContents({
            componentBundle,
            uuid,
            stream: stream.token,
        }))
            .then((buffer) => JSON.parse(buffer.toString()))
            .then((0, jsonrpc_ts_1.decode)(fdb_protocol_1.FDBTypes.AppComponentContentsList));
    }
    ...
    captureHeapSnapshot(format, uuid) {
        return this.heapSnapshotReceiver.receiveFromStream((stream) => this.beginHeapSnapshotCapture(uuid

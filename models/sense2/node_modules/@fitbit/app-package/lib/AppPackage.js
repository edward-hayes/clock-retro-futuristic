...
const extractComponentSourceMaps = (sourceMapPaths, zip) => (0, apr_map_1.default)(sourceMapPaths, (path) => getTextFromZip(zip, path).then(JSON.parse));
    ...
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        return {
            device: sourceMapManifest.device
                ? (yield (0, apr_map_1.default)(sourceMapManifest.device, (component) => extractComponentSourceMaps(component, zip)))
                : undefined,
...
function fromJSZip(fbaZip) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        const device = yield Promise.all(parser
            .getDeviceComponents()
            .map(([family, { platform, filename }]) => bufferFile(filename).then((artifact) => [
            family,
            { platform, artifact },
        ]))).then(lodash.fromPairs);
        const companionFilename = parser.getCompanionFilename();
        const companion = companionFilename
            ? yield bufferFile(companionFilename)
            : undefined;

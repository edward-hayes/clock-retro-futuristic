...
cli.use((0, buildAndInstall_1.default)({ hostConnections, appContext }));
cli.use((0, connect_1.default)({ hostConnections }));
cli.use((0, heapSnapshot_1.default)({ hostConnections, appContext }));
cli.use((0, install_1.default)({ hostConnections, appContext }));
cli.use((0, screenshot_1.default)({ hostConnections }));
cli.use((0, setAppPackage_1.default)({ appContext }));
cli.use(logout_1.default);
cli.use((0, repl_1.default)({ hostConnections }));
if (enableQACommands) {
    cli.use((0, hosts_1.default)({ hostConnections }));
    cli.use(mockHost_1.default);
}
new LogConsumer_1.default({
    appContext,
    hostConnections,
    messageFormatter: (message) => (0, formatMessage_1.formatMessage)(cli, message),
});
async function main() {
    (0, checkForUpdate_1.default)(update_notifier_1.default);
    await (0, checkLogin_1.default)();
    const user = await (0, userProfile_1.default)();
    if (user.fullName) {

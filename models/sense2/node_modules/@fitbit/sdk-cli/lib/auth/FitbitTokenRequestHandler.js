...
class FitbitTokenRequestHandler {
    ...
    performRevokeTokenRequest(configuration, request) {
        return (0, fetch_1.default)(configuration.revocationEndpoint, Object.assign(Object.assign({}, commonParams), { body: this.utils.stringify({
                token: request.token,
            }) })).then((response) => response.ok);
    }
    async performTokenRequest(configuration, request) {
        const response = await (0, fetch_1.default)(configuration.tokenEndpoint, Object.assign(Object.assign({}, commonParams), { body: this.utils.stringify(request.toStringMap()) }));

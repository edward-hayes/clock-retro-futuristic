...
const installAction = async (cli, stores, args) => {
    ...
    const appPackage = await (0, setAppPackage_1.setAppPackageAction)(cli, appContext, args.packagePath);
    if (!appPackage)
        return false;
    ...
    if (hasApp &&
        ...
        const result = await (0, connect_1.connectAction)(cli, 'device', hostConnections);
        if (!result)
            return false;
    }
    if (hasCompanion &&
        ...
        const result = await (0, connect_1.connectAction)(cli, 'phone', hostConnections);
        if (!result)
            return false;
    }
    ...
    try {
        ...
        if (hasCompanion && companionHost) {
            await sideload
                .companion(companionHost.host, appPackage, makeProgressCallback('companion'))
...
function install(stores) {
    return (cli) => {
        cli
            ...
            .action(async (args) => (0, exports.installAction)(cli, stores, args));
    };
}

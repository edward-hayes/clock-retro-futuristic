...
function screenshot(stores) {
    return (cli) => {
        cli
            .command('screenshot [path] [--open]', 'Capture a screenshot from the connected device')
            .types({ string: ['path'] })
            ...
            const destPath = path.resolve(args.path
                ...
                : (0, dateformat_1.default)('"Screenshot" yyyy-mm-dd "at" H.MM.ss."png"'));
            try {
                await (0, captureScreenshot_1.default)(appHost.host, destPath, {
                ...
                if (args.options.open) {
                    await (0, open_1.default)(destPath, { wait: false });
                }
                return true;
            }
            catch (ex) {
